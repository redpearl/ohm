...
MyGame = ig.Game.extend({
	...
	/*
	tweenColors: function(distance, maxDistance, clearColor){
		var defR = parseInt(ig.game.defaultColor.substring(1,3), 16);
		var defG = parseInt(ig.game.defaultColor.substring(3,5), 16);
		var defB = parseInt(ig.game.defaultColor.substring(5,7), 16);
		...
		this.updateColors(r, g, b);
		this.backgroundColorModified = true;
	},*/

	tweenColorsTime: function(distance, maxDistance, clearColor){
		var defR = parseInt(ig.game.currentColor.substring(1,3), 16);
		var defG = parseInt(ig.game.currentColor.substring(3,5), 16);
		var defB = parseInt(ig.game.currentColor.substring(5,7), 16);
	...
	loadGameMode: function(mode){
		if(mode === 'spark'){
			...
			ig.game.spawnEntity(EntityBlock, 500, 100);
			ig.game.spawnEntity(EntityBarrier, 750, -1500);
	...
	createParticleExplosion: function(x, y){
		...
		/*
		for (var i = 0; i < 5; i++) {
			ig.game.spawnEntity(EntityParticle, x, y, {vel: {x: 40 - i*5, y: -40 + i*5 }})
		};*/
	},
	...
	draw: function() {
		...
		if(this.tweenColorsByTime){
			if(this.colorTweenTimer.delta()<0){
				this.tweenColorsTime(Math.abs(this.colorTweenTimer.delta()), 7, ig.game.newColor);
			} else {
