...
EntityPlayer = EntityBounceable.extend({
		...
		update: function() {
			...
			if(!this.frozen){
				...
				this.accel.x = 100;
				if( ig.input.state('down') && !this.dead && !this.winner && this.forceBoostTimer.delta()>0) {
						this.go('down');
						//Moving left
						this.accel.y = 500;
						/*
				...
				} else if ( ig.input.state('up') && !this.dead && !this.winner && this.forceBoostTimer.delta()>0){
						this.go('up');
						//Moving right
						this.accel.y = -500;
						/*
				...
				if(ig.input.state('boost') && this.vel.x > 0){
					this.boosting = true;
					this.scale.y = 1.3;
					this.scale.x = 0.6;
					this.accel.x = 600;
				...
				} else if(ig.input.state('left') && this.vel.x > 50){
					this.scale.y = 1.2;
					this.scale.x = 0.8;
					this.accel.x = -600;
				} else {
