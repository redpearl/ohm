...
EntitySmallBlock = ig.Entity.extend({
		...
		blowUp: function(){
			//TweenMax.to( this.pos, 0.3, {x: this.parentBlock.pos.x + this.parentBlock.size.x/2, y: this.parentBlock.pos.y + this.parentBlock.size.y/2});
			TweenMax.to(this, 0.4, {scale: 0, onComplete: this.kill.bind(this)});
		},
		...
		update: function() {
			...
			} else if(this.tweeningColorBack && this.tweenColorTimer.delta()>0){
				this.tweeningColorBack = false;
				this.tweeningColor = false;
				// Tweening is over
				if(this.leader){
					this.parentBlock.activateTargets();
				}
			}
		},
