...
this.currentTrack.pause();if(this&&this.currentTrack){this.currentTrack.currentTime=0;}},play:function(name){if(name&&this.namedTracks[name]){var newTrack=this.namedTracks[name];if(newTrack!=this.currentTrack){this.stop();this.currentTrack=newTrack;}}
...
// lib/game/entities/bounceable.js
ig.baked=true;ig.module('game.entities.bounceable').requires('impact.entity').defines(function(){EntityBounceable=ig.Entity.extend({type:ig.Entity.TYPE.NONE,checkAgainst:ig.Entity.TYPE.NONE,collides:ig.Entity.COLLIDES.NEVER,scale:{x:1,y:1},init:function(x,y,settings){this.parent(x,y,settings);},squash:function(amount,time,bindTo){if(!time){time=0.3;}
if(!bindTo){bindTo=ig.game;}
TweenMax.killTweensOf(this);this.scale.y=1-amount;TweenMax.to(this.scale,time,{y:1,ease:Elastic.easeInOut});this.scale.x=1+amount;if(this.onComplete){TweenMax.to(this.scale,time,{x:1,ease:Elastic.easeInOut,onComplete:this.onComplete.bind(bindTo)});}else{TweenMax.to(this.scale,time,{x:1,ease:Elastic.easeInOut});}},dip:function(amount,time){this.offset.y=amount;TweenMax.to(this.offset,time,{y:0});},stretch:function(amount,time,bindTo){if(!time){time=0.3;}
if(!bindTo){bindTo=ig.game;}
TweenMax.killTweensOf(this);this.scale.y=1+amount;TweenMax.to(this.scale,time,{y:1,ease:Elastic.easeInOut});this.scale.x=1-amount;if(this.onComplete){TweenMax.to(this.scale,time,{x:1,ease:Elastic.easeInOut,onComplete:this.onComplete.bind(bindTo)});}else{TweenMax.to(this.scale,time,{x:1,ease:Elastic.easeInOut});}},update:function(){this.parent();},draw:function(){this.currentAnim.scale.x=this.scale.x;this.currentAnim.scale.y=this.scale.y;this.parent();}});});

// lib/game/entities/trailPoint.js
ig.baked=true;ig.module('game.entities.trailPoint').requires('impact.entity').defines(function(){EntityTrailPoint=ig.Entity.extend({size:{x:24,y:24},zIndex:1,gravityFactor:0,deathTime:3,dimChance:0.3,breakChance:0.1,ripple:0,type:ig.Entity.TYPE.NONE,checkAgainst:ig.Entity.TYPE.NONE,collides:ig.Entity.COLLIDES.NEVER,init:function(x,y,settings){this.parent(x,y,settings);this.lifeTimer=new ig.Timer(this.deathTime);this.rippleTimer=new ig.Timer(0.2);var rand=Math.random();if(rand<this.dimChance){this.dim=true;}
if(!this.small){this.lifeTimer=new ig.Timer(this.deathTime/3);}
ig.game.sortEntitiesDeferred();},setRipple:function(amount,dir){this.ripple=amount;this.rippleDir=dir;if(this.ripple>5){this.rippleDir=-1;}else if(this.ripple<-5){this.rippleDir=1;}else{this.rippleDir=dir;}
this.rippleTimer=new ig.Timer(0.2);},update:function(){if(this.ripple&&this.rippleTimer&&this.rippleTimer.delta()>0){if(this.otherPoint){this.otherPoint.setRipple(this.ripple,this.rippleDir);this.ripple=0;this.transparent=false;}
this.rippleTimer=false;}
this.parent();if(this.lifeTimer.delta()>0){this.kill();}},draw:function(){if(this.otherPoint&&!this.otherPoint._killed){if(!this.small){var transparency=-this.lifeTimer.delta()/(this.deathTime/3);}else{var transparency=-this.lifeTimer.delta()/this.deathTime;}
if(this.dim){transparency=transparency/2;}else if(this.transparent){transparency=0.5;}
ig.system.context.globalAlpha=transparency;ig.system.context.beginPath();if(this.small){ig.system.context.lineWidth=1;}else{ig.system.context.lineWidth=2;}
ig.system.context.strokeStyle='#f8f4f8';if(Math.abs((this.pos.y-ig.game.screen.y)*ig.system.scale-(this.otherPoint.pos.y-ig.game.screen.y)*ig.system.scale)<500){ig.system.context.moveTo((this.pos.x-ig.game.screen.x)*ig.system.scale,(this.pos.y+this.ripple-ig.game.screen.y)*ig.system.scale);ig.system.context.lineTo((this.pos.x-ig.game.screen.x)*ig.system.scale,(this.otherPoint.pos.y+this.ripple-ig.game.screen.y)*ig.system.scale);}
ig.system.context.stroke();if(!this.small){if(this.lifeTimer.delta()>-this.deathTime/2){ig.system.context.lineWidth=2;}else{ig.system.context.lineWidth=4;}}
if(this.playerTrail){if(ig.game.player.boosting){ig.system.context.strokeStyle='#f7fb7b';ig.system.context.lineWidth=8;}else if(ig.game.player.superBoosting){ig.system.context.strokeStyle='#ec914f';ig.system.context.lineWidth=12;}}
ig.system.context.beginPath();ig.system.context.moveTo((this.pos.x-ig.game.screen.x)*ig.system.scale,(this.otherPoint.pos.y+this.ripple-ig.game.screen.y)*ig.system.scale);ig.system.context.lineTo((this.otherPoint.pos.x-ig.game.screen.x)*ig.system.scale,(this.otherPoint.pos.y+this.ripple-ig.game.screen.y)*ig.system.scale);ig.system.context.stroke();ig.system.context.globalAlpha=1;}}});});

// lib/game/entities/projectile.js
ig.baked=true;ig.module('game.entities.projectile').requires('impact.entity').defines(function(){EntityProjectile=ig.Entity.extend({size:{x:6,y:6},maxVel:{x:1500,y:500},zIndex:251,gravityFactor:0,zoomRadius:120,color:'#FFFFFF',strokeColor:'#ddb7d5',lineWidth:1,scale:1,xScale:1,yScale:1,type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.A,collides:ig.Entity.COLLIDES.NEVER,init:function(x,y,settings){this.parent(x,y,settings);this.hitCooldown=new ig.Timer(0.1);this.xScale=this.scale;this.yScale=this.scale;},check:function(other){if(other===ig.game.player&&this.hitCooldown.delta()>0){}},squash:function(){this.xScale=0.5;TweenMax.to(this,0.5,{xScale:1});this.yScale=1.2;TweenMax.to(this,0.5,{yScale:1});},update:function(){this.parent();},draw:function(){ig.system.context.save();ig.system.context.translate((this.pos.x+this.size.x/2-ig.game.screen.x)*ig.system.scale,(this.pos.y+this.size.y/2-ig.game.screen.y)*ig.system.scale);ig.system.context.lineWidth=this.lineWidth;ig.system.context.fillStyle=this.color;ig.system.context.strokeStyle=this.strokeColor;ig.system.context.scale(this.xScale,this.yScale);ig.system.context.fillRect(-this.size.x/2*ig.system.scale,-this.size.y/2*ig.system.scale,this.size.x*ig.system.scale,this.size.y*ig.system.scale);ig.system.context.strokeRect(-this.size.x/2*ig.system.scale,-this.size.y/2*ig.system.scale,this.size.x*ig.system.scale,this.size.y*ig.system.scale);ig.system.context.restore();}});});

// lib/game/entities/player.js
ig.baked=true;ig.module('game.entities.player').requires('impact.entity','game.entities.bounceable','game.entities.trailPoint','game.entities.projectile').defines(function(){EntityPlayer=EntityBounceable.extend({size:{x:16,y:4},maxVel:{x:150,y:150},maximumVelocity:150,offset:{x:0,y:0},friction:{x:0,y:350},zIndex:100,gravityFactor:0,active:true,superPowerDuration:22,streak:0,dataCollected:0,type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.NONE,collides:ig.Entity.COLLIDES.PASSIVE,animSheet:new ig.AnimationSheet('media/player_sprite.png',16,4),glowImage:new ig.Image('media/player_glow.png',16,16),flip:false,accelGround:500,accelAir:500,jump:130,zipDistance:100,init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.2,[0]);this.trailPointSpawnTimer=new ig.Timer(0.1);this.rippleTimer=new ig.Timer(Math.random());this.forceBoostTimer=new ig.Timer(0);ig.game.screen.x=this.pos.x-ig.system.width/2;ig.game.screen.y=this.pos.y-ig.system.height/2;this.shotCooldown=new ig.Timer(0.4);this.shootingTimer=new ig.Timer(0);this.zipTimer=new ig.Timer(2);},freeze:function(){this.frozen=true;this.currentAnim.alpha=0;},activate:function(){this.frozen=false;this.currentAnim.alpha=1;},draw:function(){if(this.lastPoint&&!this.lastPoint._killed&&!this.frozen){ig.system.context.globalAlpha=1;ig.system.context.beginPath();ig.system.context.lineWidth=4;ig.system.context.strokeStyle='#f8f4f8';ig.system.context.moveTo((this.pos.x-ig.game.screen.x)*ig.system.scale,(this.pos.y-ig.game.screen.y)*ig.system.scale);ig.system.context.moveTo((this.pos.x-ig.game.screen.x)*ig.system.scale,(this.lastPoint.pos.y-ig.game.screen.y)*ig.system.scale);ig.system.context.lineTo((this.lastPoint.pos.x-ig.game.screen.x)*ig.system.scale,(this.lastPoint.pos.y-ig.game.screen.y)*ig.system.scale);ig.system.context.stroke();if(this.shootingTimer.delta()<0){ig.system.globalAlpha=(0.3+this.shootingTimer.delta())/0.3;ig.system.context.beginPath();ig.system.context.lineWidth=4;ig.system.context.strokeStyle='#f8f4f8';ig.system.context.moveTo((this.pos.x+this.size.x/2-ig.game.screen.x)*ig.system.scale,(this.pos.y-300-ig.game.screen.y)*ig.system.scale);ig.system.context.lineTo((this.pos.x+this.size.x/2-ig.game.screen.x)*ig.system.scale,(this.pos.y+300-ig.game.screen.y)*ig.system.scale);ig.system.context.stroke();ig.system.globalAlpha=1;}
ig.system.context.globalAlpha=1;}
this.parent();},hurt:function(enemy){if(this.superPowerTimer.delta()<0){this.streak++;this.score+=enemy.scoreValue*this.streak;ig.game.spawnEntity(EntityBoard,enemy.pos.x,enemy.pos.y,{text:enemy.scoreValue*this.streak});enemy.die();}else if(this.hurtTimer.delta()>0){this.hurtTimer=new ig.Timer(2);ig.game.sfxHurt.play();this.loseHealth();}},win:function(){this.winner=true;},loseHealth:function(){ig.game.healthBar[ig.game.healthBar.length-1].die();ig.game.healthBar.erase(ig.game.healthBar[ig.game.healthBar.length-1]);ig.game.cameraShakeTimer=new ig.Timer(0.5);if(ig.game.healthBar.length===0){this.die();}},die:function(){this.dead=true;this.deadTimer=new ig.Timer(1);ig.game.cameraShakeTimer=new ig.Timer(3);},gainSuperPower:function(){this.superPowerTimer=new ig.Timer(this.superPowerDuration);ig.music.play('super');this.isPlayingSuper=true;this.streak=0;},loseSuperPower:function(){ig.music.play('main');ig.game.loadAllCoins();ig.game.checkIfLastEnemy();},pickupCoin:function(){this.score+=10;ig.game.spawnEntity(EntityBoard,this.pos.x,this.pos.y,{text:10});ig.game.sfxCoin.play();var coins=ig.game.getEntitiesByType(EntityCoin);var last=true;coins.forEach(function(coin){if(!coin._killed){last=false;}})
if(last===true){this.gainSuperPower();}},go:function(dir){this.direction=dir;},forceBoost:function(duration){this.forceBoostTimer=new ig.Timer(duration);},collectData:function(object){this.dataCollected++;this.maximumVelocity=this.maximumVelocity+10;this.maxVel.x=this.maximumVelocity;},update:function(){if(!this.frozen){if(this.trailPointSpawnTimer.delta()>0){if(this.rippleTimer.delta()>0){var randDir=1;if(Math.random()<0.5){randDir=-1;}
this.lastPoint=ig.game.spawnEntity(EntityTrailPoint,this.pos.x+this.size.x/2,this.pos.y+this.size.y/2,{playerTrail:true,otherPoint:this.lastPoint,ripple:randDir*3*Math.random(),rippleDir:1});this.rippleTimer=new ig.Timer(Math.random()*0.5);}else{this.lastPoint=ig.game.spawnEntity(EntityTrailPoint,this.pos.x+this.size.x/2,this.pos.y+this.size.y/2,{playerTrail:true,otherPoint:this.lastPoint});}
this.trailPointSpawnTimer.reset();}
if(this.closeObject&&this.zoomValue>1){ig.game.zoomTo(this.closeObject,this.zoomValue);}else{this.closeObject=false;ig.game.centerObject=false;}
var accel=this.accelGround;if(this.dead){this.currentAnim.alpha=this.deadTimer.delta()*-1;if(this.deadTimer.delta()>0){this.currentAnim.alpha=0;this.kill();}}
this.accel.x=100;if(ig.input.state('down')&&!this.dead&&!this.winner&&this.forceBoostTimer.delta()>0){this.go('down');this.accel.y=500;if(ig.input.pressed('zip')&&this.zipTimer.delta()>0){this.zipTimer.reset();this.pos.y=this.pos.y+this.zipDistance;}}else if(ig.input.state('up')&&!this.dead&&!this.winner&&this.forceBoostTimer.delta()>0){this.go('up');this.accel.y=-500;if(ig.input.pressed('zip')&&this.zipTimer.delta()>0){this.zipTimer.reset();this.pos.y=this.pos.y-this.zipDistance;}}else{this.accel.y=0;}
if(ig.input.state('right')&&this.vel.x>0){if(!this.boosting){ig.game.sfxBoost01.play();}
if(ig.input.pressed('zip')&&this.zipTimer.delta()>0){this.zipTimer.reset();this.pos.x=this.pos.x+this.zipDistance;}
this.boosting=true;this.scale.y=1.3;this.scale.x=0.6;this.accel.x=600;this.maxVel.x=this.maximumVelocity+this.maximumVelocity/10;}else if(this.forceBoostTimer.delta()<0){this.superBoosting=true;this.scale.y=1.6;this.scale.x=0.3;this.accel.x=600;this.maxVel.x=this.maximumVelocity*2;}else if(ig.input.state('shoot')&&this.shotCooldown.delta()>0){this.scale.y=1.2;this.scale.x=0.8;this.accel.x=-600;this.shotCooldown.reset();this.shooting=true;this.shootingTimer=new ig.Timer(0.3);}else{ig.game.sfxBoost01.stop();this.boosting=false;this.superBoosting=false;this.scale.y=1;this.scale.x=1;this.maxVel.x=this.maximumVelocity;}
this.currentAnim.flip.x=this.flip;this.parent();if(this.pos.y>800){var offset=-Math.abs(ig.game.screen.y)+Math.abs(this.pos.y);this.pos.y=-500;ig.game.screen.y=-500-offset;TweenMax.killTweensOf(ig.game.screen);ig.game.setZoom(this,1,false);}else if(this.pos.y<-500){var offset=Math.abs(ig.game.screen.y)-Math.abs(this.pos.y);this.pos.y=800;ig.game.screen.y=800-offset;TweenMax.killTweensOf(ig.game.screen);ig.game.setZoom(this,1,false);}}},hit:function(direction){if(direction==='front'){this.stretch(0.3,0.3);}}});});

// lib/game/entities/particle.js
ig.baked=true;ig.module('game.entities.particle').requires('impact.entity').defines(function(){EntityParticle=EntityBounceable.extend({size:{x:4,y:4},zIndex:-5,gravityFactor:0,zoomRadius:60,timeToLive:1,scale:{x:1,y:1},type:ig.Entity.TYPE.NONE,checkAgainst:ig.Entity.TYPE.NONE,collides:ig.Entity.COLLIDES.NEVER,animSheet:new ig.AnimationSheet('media/particle.png',4,4),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',1,[0]);this.addAnim('white',1,[1]);this.lifeTime=new ig.Timer(0);TweenMax.to(this.scale,2,{x:0,y:0});this.rotateFactor=Math.random();this.vel.x=this.vel.x+Math.random();this.vel.y=this.vel.y+Math.random();if(this.color==='white'){this.currentAnim=this.anims.white;}},update:function(){this.parent();if(this.lifeTime.delta()>this.timeToLive){this.kill();}
this.currentAnim.angle+=this.rotateFactor;this.currentAnim.alpha=1-this.lifeTime.delta();},draw:function(){this.parent();}});});

// lib/game/entities/nonPlayer.js
ig.baked=true;ig.module('game.entities.nonPlayer').requires('impact.entity').defines(function(){EntityNonPlayer=ig.Entity.extend({size:{x:2,y:2},zIndex:251,maxVel:{x:200,y:100},gravityFactor:0,deathTime:3,dimChance:0.3,breakChance:0.1,brokenOffset:0,type:ig.Entity.TYPE.NONE,checkAgainst:ig.Entity.TYPE.NONE,collides:ig.Entity.COLLIDES.NEVER,animSheet:new ig.AnimationSheet('media/player_sprite.png',2,2),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.2,[0]);this.trailPointSpawnTimer=new ig.Timer(0.8);this.vel.x=ig.game.player.vel.x+Math.random()*50;this.lifeTimer=new ig.Timer(8);this.currentAnim.alpha=0.6;},update:function(){this.parent();if(this.lifeTimer.delta()>0){this.kill();}
if(this.trailPointSpawnTimer.delta()>0){this.lastPoint=ig.game.spawnEntity(EntityTrailPoint,this.pos.x+this.size.x/2,this.pos.y+this.size.y/2,{otherPoint:this.lastPoint,small:true});this.trailPointSpawnTimer.reset();}},draw:function(){this.parent();if(this.lastPoint&&!this.lastPoint._killed){var transparency=-this.lifeTimer.delta()/this.deathTime;ig.system.context.globalAlpha=transparency;ig.system.context.beginPath();ig.system.context.lineWidth=1;ig.system.context.strokeStyle='#f8f4f8';ig.system.context.moveTo((this.pos.x-ig.game.screen.x)*ig.system.scale,(this.pos.y+this.brokenOffset-ig.game.screen.y)*ig.system.scale);ig.system.context.lineTo((this.pos.x-ig.game.screen.x)*ig.system.scale,(this.lastPoint.pos.y+this.brokenOffset-ig.game.screen.y)*ig.system.scale);ig.system.context.lineTo((this.lastPoint.pos.x-ig.game.screen.x)*ig.system.scale,(this.lastPoint.pos.y+this.brokenOffset-ig.game.screen.y)*ig.system.scale);ig.system.context.stroke();ig.system.context.globalAlpha=1;}}});});

// lib/game/entities/smallBlock.js
ig.baked=true;ig.module('game.entities.smallBlock').requires('impact.entity').defines(function(){EntitySmallBlock=ig.Entity.extend({size:{x:24,y:24},zIndex:251,gravityFactor:0,type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.A,collides:ig.Entity.COLLIDES.NEVER,colors:['#f7fb7b','#f1d472','#ecbc70','#18012a'],tweenDuration:1,scale:1,animSheet:new ig.AnimationSheet('media/small_block.png',24,24),init:function(x,y,settings){this.parent(x,y,settings);var frame=Math.round(Math.random()*3);this.addAnim('idle',0.2,[frame]);this.color=this.colors[Math.round(Math.random()*(this.colors.length-1))];this.originalColor=this.color;var rotation=Math.random()/7;TweenMax.to(this.currentAnim,0.7,{angle:rotation,onComplete:this.rotateBack.bind(this)});TweenMax.to(this.pos,0.8,{x:this.pos.x+this.mod.x,y:this.pos.y+this.mod.y,onComplete:this.moveBack.bind(this)});},tweenColor:function(){this.tweenColorTimer=new ig.Timer(this.tweenDuration);this.newColor='#f8f4f8';this.tweeningColor=true;},tweenColorBack:function(){this.tweenDuration=0.5;this.newColor=this.originalColor;this.originalColor=this.color;this.tweenColorTimer=new ig.Timer(this.tweenDuration);this.tweeningColor=true;},rotateBack:function(){TweenMax.to(this.currentAnim,0.25,{angle:0});},blowUp:function(){TweenMax.to(this,0.4,{scale:0,onComplete:this.kill.bind(this)});},moveBack:function(){TweenMax.to(this.pos,0.25,{x:this.pos.x-this.mod.x,y:this.pos.y-this.mod.y,onComplete:this.glow.bind(this)});},glow:function(){this.tweenColor();},update:function(){this.currentAnim.update();if(this.tweenColorTimer&&this.tweenColorTimer.delta()<0&&this.tweeningColor){this.color=ig.game.tweenColorsTime(this.tweenColorTimer.delta()+this.tweenDuration,this.tweenDuration,this.newColor,this.originalColor);}else if(this.tweeningColor&&!this.tweeningColorBack){this.tweenColorBack();this.tweeningColorBack=true;this.color=this.originalColor;}else if(this.tweeningColorBack&&this.tweenColorTimer.delta()>0){this.tweeningColorBack=false;this.tweeningColor=false;if(this.leader){this.parentBlock.activateTargets();}}},draw:function(){ig.system.context.save();ig.system.context.translate((this.pos.x+this.size.x/2-ig.game.screen.x)*ig.system.scale,(this.pos.y+this.size.y/2-ig.game.screen.y)*ig.system.scale);ig.system.context.rotate(this.currentAnim.angle);ig.system.context.fillStyle=this.color;ig.system.context.scale(this.scale,this.scale);ig.system.context.fillRect(-this.size.x/2*ig.system.scale,-this.size.y/2*ig.system.scale,this.size.x*ig.system.scale,this.size.y*ig.system.scale);ig.system.context.restore();}});});

// lib/game/entities/block.js
ig.baked=true;ig.module('game.entities.block').requires('impact.entity','game.entities.smallBlock').defines(function(){EntityBlock=EntityBounceable.extend({size:{x:48,y:48},zIndex:251,gravityFactor:0,zoomRadius:100,type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.A,collides:ig.Entity.COLLIDES.NEVER,sysText:'HIT PPU\nApply Power',animSheet:new ig.AnimationSheet('media/block.png',48,48),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.2,[0]);this.hitCooldown=new ig.Timer(0.1);this.cameraReleaseTimer=new ig.Timer(0);},check:function(other){if(other===ig.game.player&&this.hitCooldown.delta()>0&&!ig.game.player.boosting&&!this.hasSplit){ig.game.player.vel.x=-ig.game.player.vel.x+-ig.game.player.vel.x/10;this.hitCooldown.reset();ig.game.player.hit('front');if(this.plaque){this.plaque.remove();}
this.plaque=ig.game.spawnEntity(EntityTextPlaque,other.pos.x-100,other.pos.y-50,{text:this.sysText,origin:{x:other.pos.x+other.size.x,y:other.pos.y+other.size.y/2}});}else if(other===ig.game.player&&this.hitCooldown.delta()>0&&!this.hasSplit){this.hasSplit=true;ig.game.player.freeze();this.split();this.currentAnim.alpha=0;ig.game.sfxSuperBoom.play();ig.game.tweenColors(ig.game.mainWhiteColor,0.5,ig.game.resetColors);}},activateTargets:function(){var target=ig.game.getEntityByName(this.targetName);target.activate(this);this.releaseCamera();},blowUp:function(){this.currentAnim.alpha=0;this.blocks.forEach(function(smallBlock){smallBlock.blowUp();})},releaseCamera:function(){this.cameraReleaseTimer=new ig.Timer(4);},split:function(){this.blocks=[];this.blocks.push(ig.game.spawnEntity(EntitySmallBlock,this.pos.x,this.pos.y,{parentBlock:this,mod:{x:-2,y:-2}}));this.blocks.push(ig.game.spawnEntity(EntitySmallBlock,this.pos.x+24,this.pos.y,{parentBlock:this,mod:{x:2,y:-2}}));this.blocks.push(ig.game.spawnEntity(EntitySmallBlock,this.pos.x,this.pos.y+24,{parentBlock:this,mod:{x:-2,y:2}}));this.blocks.push(ig.game.spawnEntity(EntitySmallBlock,this.pos.x+24,this.pos.y+24,{parentBlock:this,leader:true,mod:{x:2,y:2}}));},update:function(){if((this.cameraReleaseTimer.delta()>0)&&(this.distanceTo(ig.game.player)<this.zoomRadius||this===ig.game.player.closeObject)){ig.game.player.closeObject=this;ig.game.player.zoomValue=this.zoomRadius/this.distanceTo(ig.game.player);}
this.currentAnim.update();},draw:function(){this.parent();}});});

// lib/game/entities/dataPacket.js
ig.baked=true;ig.module('game.entities.dataPacket').requires('impact.entity').defines(function(){EntityDataPacket=ig.Entity.extend({size:{x:12,y:12},zIndex:251,gravityFactor:0,zoomRadius:120,color:'#2b031d',strokeColor:'#ddb7d5',lineWidth:4,scale:1,xScale:1,yScale:1,type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.A,collides:ig.Entity.COLLIDES.NEVER,init:function(x,y,settings){this.parent(x,y,settings);this.hitCooldown=new ig.Timer(0.1);this.timeStep=new ig.Timer(60/124);},check:function(other){if(other===ig.game.player&&this.hitCooldown.delta()>0&&!this.removed){this.remove();ig.game.sfxArrow01.play();ig.game.player.collectData(this);}},remove:function(){this.timeStep=new ig.Timer(222);TweenMax.to(this,0.2,{scale:0,onComplete:this.kill.bind(this)})
this.removed=true;},squash:function(){this.xScale=0.5;TweenMax.to(this,0.5,{xScale:1});this.yScale=1.2;TweenMax.to(this,0.5,{yScale:1});},update:function(){if(this.timeStep.delta()>0){this.timeStep.reset();if(this.scale===0.5){this.scale=1;}else{this.scale=0.5;}}
this.xScale=this.scale;this.yScale=this.scale;},draw:function(){ig.system.context.save();ig.system.context.translate((this.pos.x+this.size.x/2-ig.game.screen.x)*ig.system.scale,(this.pos.y+this.size.y/2-ig.game.screen.y)*ig.system.scale);ig.system.context.lineWidth=this.lineWidth;ig.system.context.fillStyle=this.color;ig.system.context.strokeStyle=this.strokeColor;ig.system.context.scale(this.xScale,this.yScale);ig.system.context.fillRect(-this.size.x/2*ig.system.scale,-this.size.y/2*ig.system.scale,this.size.x*ig.system.scale,this.size.y*ig.system.scale);ig.system.context.strokeRect(-this.size.x/2*ig.system.scale,-this.size.y/2*ig.system.scale,this.size.x*ig.system.scale,this.size.y*ig.system.scale);ig.system.context.restore();}});});

// lib/game/entities/platform.js
ig.baked=true;ig.module('game.entities.platform').requires('impact.entity').defines(function(){EntityPlatform=ig.Entity.extend({size:{x:12,y:48},zIndex:251,gravityFactor:0,zoomRadius:120,color:'#2b031d',strokeColor:'#ddb7d5',lineWidth:4,xScale:1,yScale:1,type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.A,collides:ig.Entity.COLLIDES.NEVER,animSheet:new ig.AnimationSheet('media/block.png',12,48),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.2,[0]);this.hitCooldown=new ig.Timer(0.1);},check:function(other){if(other===ig.game.player&&this.hitCooldown.delta()>0){ig.game.player.vel.x=-ig.game.player.vel.x+-ig.game.player.vel.x/10;this.hitCooldown.reset();ig.game.player.hit('front');if(ig.game.player.pos.x>this.pos.x){ig.game.createSmallParticleExplosion(other.pos.x+other.size.x/2+2,other.pos.y+other.size.y/2,1);}else{ig.game.createSmallParticleExplosion(other.pos.x+other.size.x/2+2,other.pos.y+other.size.y/2,-1);}
ig.game.sfxHit.play();this.squash();}},squash:function(){this.xScale=0.5;TweenMax.to(this,0.5,{xScale:1});this.yScale=1.2;TweenMax.to(this,0.5,{yScale:1});},update:function(){this.currentAnim.update();},draw:function(){ig.system.context.save();ig.system.context.translate((this.pos.x+this.size.x/2-ig.game.screen.x)*ig.system.scale,(this.pos.y+this.size.y/2-ig.game.screen.y)*ig.system.scale);ig.system.context.lineWidth=this.lineWidth;ig.system.context.fillStyle=this.color;ig.system.context.strokeStyle=this.strokeColor;ig.system.context.scale(this.xScale,this.yScale);ig.system.context.fillRect(-this.size.x/2*ig.system.scale,-this.size.y/2*ig.system.scale,this.size.x*ig.system.scale,this.size.y*ig.system.scale);ig.system.context.strokeRect(-this.size.x/2*ig.system.scale,-this.size.y/2*ig.system.scale,this.size.x*ig.system.scale,this.size.y*ig.system.scale);ig.system.context.restore();}});});

// lib/game/entities/textPlaque.js
ig.baked=true;ig.module('game.entities.textPlaque').requires('impact.entity').defines(function(){EntityTextPlaque=ig.Entity.extend({size:{x:100,y:30},zIndex:251,gravityFactor:0,zoomRadius:60,text:'Placeholder.',revealedText:'',cursor:'_',blinkTime:0.5,revealRatio:0.025,charactersWritten:0,alpha:0.8,type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.A,collides:ig.Entity.COLLIDES.NEVER,init:function(x,y,settings){this.parent(x,y,settings);this.blinkTimer=new ig.Timer(this.blinkTime);this.textTimer=new ig.Timer(0);this.calculatePosition();},remove:function(){TweenMax.to(this,0.5,{alpha:0,onComplete:this.kill.bind(this)});},update:function(){this.calculateRevealedText();},appendText:function(text){this.charactersWritten=this.text.length;this.text=this.text+text;this.textTimer=new ig.Timer(-this.charactersWritten*this.revealRatio);},calculatePosition:function(){var y=ig.game.player.pos.y-100;var x=this.origin.x-120;TweenMax.to(this.pos,1,{x:x,y:y});},draw:function(){ig.game.font_small.alpha=this.alpha;if(this.blinkTimer.delta()>this.blinkTime){ig.game.font_small.draw(this.revealedText,this.pos.x-ig.game.screen.x+10,this.pos.y-ig.game.screen.y+15);this.blinkTimer.reset();}else if(this.blinkTimer.delta()>0){ig.game.font_small.draw(this.revealedText+this.cursor,this.pos.x-ig.game.screen.x+10,this.pos.y-ig.game.screen.y+15);}else{ig.game.font_small.draw(this.revealedText,this.pos.x-ig.game.screen.x+10,this.pos.y-ig.game.screen.y+15);}
if(this.revealed&&!this.killTimer){this.killTimer=new ig.Timer(5);}
ig.system.context.globalAlpha=this.alpha;ig.system.context.strokestyle='#000000';ig.system.context.strokeRect((this.pos.x-ig.game.screen.x)*ig.system.scale,(this.pos.y-ig.game.screen.y+10)*ig.system.scale,this.size.x*ig.system.scale,this.size.y*ig.system.scale);ig.system.context.beginPath();ig.system.context.moveTo((this.pos.x-ig.game.screen.x)*ig.system.scale+this.size.x*ig.system.scale,(this.pos.y-ig.game.screen.y+10)*ig.system.scale+this.size.y*ig.system.scale);ig.system.context.lineTo((this.origin.x-ig.game.screen.x)*ig.system.scale,(this.origin.y-ig.game.screen.y)*ig.system.scale);ig.system.context.stroke();ig.system.context.globalAlpha=1;if(this.killTimer&&this.killTimer.delta()>0){this.kill();}
this.parent();ig.game.font_small.alpha=1;},calculateRevealedText:function(){var revealedChars=Math.floor(this.textTimer.delta()/this.revealRatio);if(revealedChars>this.charactersWritten&&revealedChars<=this.text.length){this.charactersWritten=revealedChars;}
if(revealedChars>=this.text.length){this.revealedText=this.text;this.revealed=true;}else{this.revealedText=this.text.substr(0,revealedChars);}}});});

// lib/game/entities/barrier.js
ig.baked=true;ig.module('game.entities.barrier').requires('impact.entity','game.entities.textPlaque').defines(function(){EntityBarrier=ig.Entity.extend({size:{x:12,y:6000},zIndex:-10,gravityFactor:0,zoomRadius:60,darkColor:'#100710',mediumDarkColor:'#320221',sysText:'HIT Firewall\nToo low SPD',type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.A,collides:ig.Entity.COLLIDES.NEVER,color:'#ddb7d5',init:function(x,y,settings){this.parent(x,y,settings);this.hitCooldown=new ig.Timer(0.1);},check:function(other){if(other===ig.game.player&&this.hitCooldown.delta()>0&&!ig.game.player.superBoosting&&!this.broken){ig.game.player.vel.x=-ig.game.player.vel.x+-ig.game.player.vel.x/10;this.hitCooldown.reset();ig.game.player.hit('front');ig.game.createSmallParticleExplosion(this.pos.x,this.pos.y);if(!this.appended){this.appended=true;}
if(this.plaque){this.plaque.remove();}
this.plaque=ig.game.spawnEntity(EntityTextPlaque,other.pos.x-150,other.pos.y-50,{text:this.sysText,origin:{x:other.pos.x+other.size.x,y:other.pos.y+other.size.y/2}});}else if(other===ig.game.player&&!this.broken&&ig.game.player.superBoosting){this.broken=true;ig.game.sfxSuperBoom.play();ig.game.switchMusicTrack('epic',0);ig.game.activateSlowMotion(1);ig.game.startSpawningDataPackets();ig.game.createParticleExplosion(this.pos.x+2,other.pos.y+other.size.y/2);this.alpha=0.3;TweenMax.to(this,1,{alpha:1});}},isOnScreen:function(){if((this.pos.x>ig.game.screen.x&&this.pos.x<ig.game.screen.x+ig.system.width)||(this.pos.x+this.size.x>ig.game.screen.x&&this.pos.x+this.size.x>ig.game.screen.x+ig.system.width)){return true;}else if((this.pos.y>ig.game.screen.y&&this.pos.y<ig.game.screen.y+ig.system.height)||(this.pos.y+this.size.y>ig.game.screen.y&&this.pos.y+this.size.y>ig.game.screen.y+ig.system.height)){return true;}else{return false;}},update:function(){if(this.distanceTo(ig.game.player)<this.zoomRadius||this===ig.game.player.closeObject){ig.game.player.closeObject=this;ig.game.player.zoomValue=this.zoomRadius/this.distanceTo(ig.game.player);}},draw:function(){if(this.isOnScreen()||ig.editor){ig.system.context.globalAlpha=1;ig.system.context.fillStyle=this.darkColor;ig.system.context.fillRect((this.pos.x-ig.game.screen.x)*ig.system.scale,(this.pos.y-ig.game.screen.y)*ig.system.scale,3000,6000);ig.system.context.fillStyle=this.mediumDarkColor;ig.system.context.globalAlpha=0.8;if(!ig.editor){ig.system.context.fillRect((this.pos.x-ig.game.screen.x)*ig.system.scale,(this.pos.y-ig.game.screen.y)*ig.system.scale,150-(this.pos.x-ig.game.player.pos.x)/4,6000);}else{ig.system.context.fillRect((this.pos.x-ig.game.screen.x)*ig.system.scale,(this.pos.y-ig.game.screen.y)*ig.system.scale,150-(400)/4,6000);}
ig.system.context.globalAlpha=1;ig.system.context.fillStyle=this.color;ig.system.context.fillRect((this.pos.x-ig.game.screen.x)*ig.system.scale,(this.pos.y-ig.game.screen.y)*ig.system.scale,4,6000);}
this.parent();}});});

// lib/game/entities/storyText.js
ig.baked=true;ig.module('game.entities.storyText').requires('impact.entity').defines(function(){EntityStoryText=ig.Entity.extend({size:{x:480,y:320},zIndex:251,gravityFactor:0,zoomRadius:60,text:'Systems online.',revealedText:'',cursor:'_',blinkTime:0.5,revealRatio:0.25,charactersWritten:0,type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.A,collides:ig.Entity.COLLIDES.NEVER,init:function(x,y,settings){this.parent(x,y,settings);this.blinkTimer=new ig.Timer(this.blinkTime);this.textTimer=new ig.Timer(0);},update:function(){this.parent();this.calculateRevealedText();},appendText:function(text){this.charactersWritten=this.text.length;this.text=this.text+text;this.textTimer=new ig.Timer(-this.charactersWritten*this.revealRatio);},draw:function(){if(this.blinkTimer.delta()>this.blinkTime){ig.game.font.draw(this.revealedText,this.pos.x+10,this.pos.y+10);this.blinkTimer.reset();}else if(this.blinkTimer.delta()>0){ig.game.font.draw(this.revealedText+this.cursor,this.pos.x+10,this.pos.y+10);}else{ig.game.font.draw(this.revealedText,this.pos.x+10,this.pos.y+10);}
this.parent();},calculateRevealedText:function(){var revealedChars=Math.floor(this.textTimer.delta()/this.revealRatio);if(revealedChars>this.charactersWritten&&revealedChars<=this.text.length){ig.game.playRandomTextSfx();this.charactersWritten=revealedChars;}
if(revealedChars>=this.text.length){this.revealedText=this.text;this.revealed=true;}else{this.revealedText=this.text.substr(0,revealedChars);}}});});

// lib/game/entities/startMenu.js
ig.baked=true;ig.module('game.entities.startMenu').requires('impact.entity').defines(function(){EntityStartMenu=ig.Entity.extend({size:{x:480,y:320},zIndex:251,gravityFactor:0,zoomRadius:60,text:'Start?',revealedText:'',cursor:'_',blinkTime:0.5,revealRatio:0.4,charactersWritten:0,type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.A,collides:ig.Entity.COLLIDES.NEVER,init:function(x,y,settings){this.parent(x,y,settings);this.blinkTimer=new ig.Timer(this.blinkTime);this.textTimer=new ig.Timer(0);},update:function(){this.parent();this.calculateRevealedText();if(ig.input.pressed('boost')){ig.game.loadGameMode('text');}},draw:function(){if(this.blinkTimer.delta()>this.blinkTime){ig.game.font.draw(this.revealedText,this.pos.x-ig.game.screen.x+10,this.pos.y-ig.game.screen.y+10);this.blinkTimer.reset();}else if(this.blinkTimer.delta()>0){ig.game.font.draw(this.revealedText+this.cursor,this.pos.x-ig.game.screen.x+10,this.pos.y-ig.game.screen.y+10);}else{ig.game.font.draw(this.revealedText,this.pos.x-ig.game.screen.x+10,this.pos.y-ig.game.screen.y+10);}
this.parent();},calculateRevealedText:function(){var revealedChars=Math.floor(this.textTimer.delta()/this.revealRatio);if(revealedChars>this.charactersWritten&&revealedChars<=this.text.length){ig.game.playRandomTextSfx();this.charactersWritten=revealedChars;}
if(revealedChars>=this.text.length){this.revealedText=this.text;this.revealed=true;}else{this.revealedText=this.text.substr(0,revealedChars);}}});});

// lib/game/entities/debris.js
ig.baked=true;ig.module('game.entities.debris').requires('impact.entity','game.entities.bounceable').defines(function(){EntityDebris=EntityBounceable.extend({size:{x:24,y:24},zIndex:1,gravityFactor:0,zoomRadius:60,maxVel:{x:3000,y:300},type:ig.Entity.TYPE.NONE,checkAgainst:ig.Entity.TYPE.NONE,collides:ig.Entity.COLLIDES.NEVER,animSheet:new ig.AnimationSheet('media/debris.png',24,24),animSheet2:new ig.AnimationSheet('media/debris2.png',24,24),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.2,[0]);this.currentAnim.alpha=0.25;if(Math.random()>0.5){this.animSheet=this.animSheet2;this.addAnim('idle',0.2,[0]);this.currentAnim.alpha=0.15;}
this.randomCooldown=new ig.Timer(Math.random()*3);this.lifeTimer=new ig.Timer(3);this.vel.x=-700-1500*Math.random();this.scale.x=4*Math.random();this.scale.y=this.scale.x;},update:function(){this.parent();if(this.lifeTimer.delta()>0){this.kill();}},draw:function(){this.parent();}});});

// lib/game/levels/test.js
ig.baked=true;ig.module('game.levels.test').requires('impact.image').defines(function(){LevelTest={"entities":[],"layer":[]};});

// lib/game/entities/arrow.js
ig.baked=true;ig.module('game.entities.arrow').requires('impact.entity').defines(function(){EntityArrow=ig.Entity.extend({size:{x:18,y:24},zIndex:251,gravityFactor:0,color:'#f7fb7b',active:'true',alpha:1,type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.A,collides:ig.Entity.COLLIDES.NEVER,animSheet:new ig.AnimationSheet('media/arrow_medium.png',18,24),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.2,[0]);this.currentAnim.flip.x=true;this.hitCooldown=new ig.Timer(1);},check:function(other){if(other===ig.game.player&&this.active=='true'){other.forceBoost(1.5);this.hitCooldown.reset();ig.game.activateSlowMotion(0.05);}},activate:function(parentActivator){if(parentActivator){this.parentActivator=parentActivator;}
this.alpha=0;this.active='true';this.takeCameraFocus();ig.game.playRandomArrowSfx();},takeCameraFocus:function(){if(!this.nextName){ig.game.zoomTo(this,2,0.35,false,this.done.bind(this,this.parentActivator));}else{ig.game.zoomTo(this,2,0.35,false,this.doneNext.bind(this,this.parentActivator));}
TweenMax.to(this,0.25,{alpha:1});},done:function(parentActivator){this.parentActivator=parentActivator;ig.game.zoomTo(ig.game.player,2,1,false,ig.game.player.activate.bind(ig.game.player));this.parentActivator.blowUp();},doneNext:function(parentActivator){this.parentActivator=parentActivator;var entity=ig.game.getEntityByName(this.nextName);ig.game.zoomTo(entity,2,0.35,false,entity.activate.bind(entity,parentActivator));},update:function(){if(this.active=='true'){this.currentAnim.update();}},draw:function(){if(this.active=='true'){ig.system.context.save();ig.system.context.globalAlpha=this.alpha;ig.system.context.translate((this.pos.x+this.size.x/2-ig.game.screen.x)*ig.system.scale,(this.pos.y+this.size.y/2-ig.game.screen.y)*ig.system.scale);ig.system.context.fillStyle=this.color;ig.system.context.rotate(0.8);ig.system.context.fillRect(-this.size.x/2*ig.system.scale,-this.size.y/2*ig.system.scale,16*ig.system.scale,10*ig.system.scale);ig.system.context.rotate(1.60);ig.system.context.translate(-6,9);ig.system.context.fillRect(-this.size.x/2*ig.system.scale,-this.size.y/2*ig.system.scale,16*ig.system.scale,10*ig.system.scale);ig.system.context.restore();}}});});

// lib/game/levels/level00.js
ig.baked=true;ig.module('game.levels.level00').requires('impact.image','game.entities.barrier','game.entities.platform','game.entities.arrow').defines(function(){LevelLevel00={"entities":[{"type":"EntityBarrier","x":676,"y":-2024},{"type":"EntityPlatform","x":60,"y":68},{"type":"EntityPlatform","x":364,"y":-100},{"type":"EntityPlatform","x":488,"y":-12},{"type":"EntityPlatform","x":164,"y":-176},{"type":"EntityPlatform","x":256,"y":-12},{"type":"EntityArrow","x":596,"y":-84},{"type":"EntityPlatform","x":592,"y":-192}],"layer":[]};});

// lib/game/levels/level01.js
ig.baked=true;ig.module('game.levels.level01').requires('impact.image','game.entities.barrier','game.entities.block','game.entities.platform','game.entities.arrow').defines(function(){LevelLevel01={"entities":[{"type":"EntityBarrier","x":676,"y":-1304},{"type":"EntityBlock","x":460,"y":-272,"settings":{"targetName":"arrow"}},{"type":"EntityPlatform","x":616,"y":-60},{"type":"EntityPlatform","x":560,"y":-120},{"type":"EntityPlatform","x":508,"y":-192},{"type":"EntityArrow","x":540,"y":-260,"settings":{"name":"arrow","active":"false","nextName":"arrow2"}},{"type":"EntityArrow","x":572,"y":-260,"settings":{"name":"arrow2","active":"false","nextName":"arrow3"}},{"type":"EntityArrow","x":604,"y":-260,"settings":{"name":"arrow3","active":"false","nextName":"arrow4"}},{"type":"EntityArrow","x":636,"y":-260,"settings":{"name":"arrow4","active":"false"}}],"layer":[]};});

// lib/plugins/scalingPlugin.js
ig.baked=true;ig.module('plugins.scalingPlugin').requires('impact.image','impact.animation','impact.entity').defines(function()
...
ig.Image.drawCount++;}}),ig.Animation.inject({scale:{x:1,y:1},draw:function(targetX,targetY)
{if(!this.scale.x){this.scale.x=1;}
if(!this.scale.y){this.scale.y=1;}
var bbsize=Math.max(this.sheet.width,this.sheet.height);if(targetX>ig.system.width||targetY>ig.system.height||targetX+bbsize<0||targetY+bbsize<0){return;}
if(this.alpha!=1){ig.system.context.globalAlpha=this.alpha;}
if(this.angle==0){this.sheet.image.drawTile(targetX,targetY,this.tile,this.sheet.width,this.sheet.height,this.flip.x,this.flip.y,this.scale.x,this.scale.y);}
else{ig.system.context.save();ig.system.context.translate(ig.system.getDrawPos(targetX+this.pivot.x),ig.system.getDrawPos(targetY+this.pivot.y));ig.system.context.rotate(this.angle);this.sheet.image.drawTile(-this.pivot.x,-this.pivot.y,this.tile,this.sheet.width,this.sheet.height,this.flip.x,this.flip.y,this.scale.x,this.scale.y);ig.system.context.restore();}
if(this.alpha!=1){ig.system.context.globalAlpha=1;}}}),ig.Entity.inject({scale:{x:1,y:1},draw:function()
{if(this.currentAnim){this.currentAnim.scale.x=this.scale.x;this.currentAnim.scale.y=this.scale.y;this.currentAnim.draw(this.pos.x-this.offset.x-ig.game._rscreen.x,this.pos.y-this.offset.y-ig.game._rscreen.y);}}});});
...
// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.font','game.entities.player','game.entities.particle','game.entities.nonPlayer','game.entities.block','game.entities.dataPacket','game.entities.platform','game.entities.barrier','game.entities.storyText','game.entities.startMenu','game.entities.debris','game.levels.test','game.levels.level00','game.levels.level01','plugins.scalingPlugin','plugins.gamepads').defines(function(){MyGame=ig.Game.extend({font:new ig.Font('media/courier16_white.png'),font_small:new ig.Font('media/courier_8_white.png'),big:false,scaleFactor:1,tweenColorsByTime:false,clearColor:'#af0c27',currentColor:'#af0c27',newColor:'#af0c27',defaultColor:'#af0c27',darkColor:'#100710',mediumDarkColor:'#320221',mainRedColor:'#af0c27',mainWhiteColor:'#f8f4f8',init:function(){ig.Sound.enabled=true;ig.game.initKeys();ig.game.loadMusicTracks();ig.game.loadSoundEffects();ig.game.slowMotionTimer=new ig.Timer(0);ig.game.loadGameMode('spark');},loadMusicTracks:function(){ig.music.add('media/music/atmospheric2.*','atmospheric2');ig.music.add('media/music/epic.*','epic');ig.music.loop=true;ig.music.volume=1;},switchMusicTrack:function(musicName,time){TweenMax.to(ig.music,time,{volume:0,onComplete:ig.game.playMusicAndFadeUpVolume.bind(ig.game,musicName,time)});},playMusicAndFadeUpVolume:function(musicName,time){ig.music.play(musicName);TweenMax.to(ig.music,time,{volume:1});},loadSoundEffects:function(){ig.game.sfxHit=new ig.Sound('media/sound/sfx_hit.*');ig.game.sfxHit.volume=0.5;ig.game.sfxBoom=new ig.Sound('media/sound/boom.*');ig.game.sfxShortBoom=new ig.Sound('media/sound/short_boom.*');ig.game.sfxSuperBoom=new ig.Sound('media/sound/super_boom.*');ig.game.sfxText01=new ig.Sound('media/sound/sfx_text_01.*');ig.game.sfxText02=new ig.Sound('media/sound/sfx_text_02.*');ig.game.sfxText03=new ig.Sound('media/sound/sfx_text_03.*');ig.game.sfxText04=new ig.Sound('media/sound/sfx_text_04.*');ig.game.sfxText05=new ig.Sound('media/sound/sfx_text_05.*');ig.game.sfxText06=new ig.Sound('media/sound/sfx_text_06.*');ig.game.textSfx=[ig.game.sfxText01,ig.game.sfxText02,ig.game.sfxText03,ig.game.sfxText04,ig.game.sfxText05,ig.game.sfxText06];ig.game.sfxBoost01=new ig.Sound('media/sound/sfx_fullspeed.*');ig.game.sfxBoost02=new ig.Sound('media/sound/sfx_holdingboost.*');ig.game.sfxArrow01=new ig.Sound('media/sound/sfx_arrow1.*');ig.game.sfxArrow02=new ig.Sound('media/sound/sfx_arrow2.*');ig.game.sfxArrow03=new ig.Sound('media/sound/sfx_arrow3.*');ig.game.sfxArrow04=new ig.Sound('media/sound/sfx_arrow4.*');ig.game.arrowSfx=[ig.game.sfxArrow01,ig.game.sfxArrow04];},playRandomTextSfx:function(){ig.game.textSfx[Math.round((ig.game.textSfx.length-1)*Math.random())].play();},playRandomArrowSfx:function(){ig.game.arrowSfx[Math.round((ig.game.arrowSfx.length-1)*Math.random())].play();},translateColorToString:function(r,g,b){var rr=r.toString(16);var gg=g.toString(16);var bb=b.toString(16);if(rr.length<2){rr='0'+rr;}else if(gg.length<2){gg='0'+gg;}else if(bb.length<2){bb='0'+bb;}
var color='#'+rr+gg+bb;return color;},tweenColors:function(newColor,time,callback){this.tweenColorsByTime=true;this.tweenColorDuration=time;this.newColor=newColor;this.colorTweenTimer=new ig.Timer(time);ig.game.colorTweenCallback=callback;},startSpawningDataPackets:function(){ig.game.dataPacketTimer=new ig.Timer(0.5);},tweenColorsTime:function(distance,maxDistance,currentColor,clearColor){var defR=parseInt(currentColor.substring(1,3),16);var defG=parseInt(currentColor.substring(3,5),16);var defB=parseInt(currentColor.substring(5,7),16);var newR=parseInt(clearColor.substring(1,3),16);var newG=parseInt(clearColor.substring(3,5),16);var newB=parseInt(clearColor.substring(5,7),16);var ratio=(maxDistance-distance)/maxDistance;var r=Math.floor(defR+(-defR+newR)*ratio);var g=Math.floor(defG+(-defG+newG)*ratio);var b=Math.floor(defB+(-defB+newB)*ratio);var sanitycheck=String.valueOf(r).length;if(r.toString().length<2){r='0'+r;}else if(g.toString().length<2){g='0'+g;}else if(b.toString().length<2){b='0'+b;}
return this.translateColorToString(r,g,b);},resetColors:function(){ig.game.tweenColors(ig.game.mainRedColor,0.5,false);},loadGameMode:function(mode){if(mode==='spark'){ig.music.play('atmospheric2');ig.game.mode=mode;ig.game.scaleFactor=1;ig.game.zoomed=false;this.currentColor='#af0c27';ig.game.loadLevel(LevelLevel01);ig.game.player=ig.game.spawnEntity(EntityPlayer,100,100);ig.game.spawnEntity(EntityDataPacket,500,100);ig.game.backgroundSpawnTimer=new ig.Timer(1.5);}else if(mode==='text'){ig.game.mode=mode;this.currentColor='#000000';ig.game.loadLevel(LevelTest);ig.game.spawnEntity(EntityStoryText,0,0);}else{ig.game.mode='intro';this.currentColor='#000000';ig.game.loadLevel(LevelTest);ig.game.spawnEntity(EntityStartMenu,0,0);}},createParticleExplosion:function(x,y){var velocity=35;for(var i=9;i>0;i--){ig.game.spawnEntity(EntityParticle,x,y,{scale:{x:0.5,y:0.5},vel:{x:velocity*Math.cos(-0.2*i),y:velocity*Math.sin(-0.2*i)}})};for(var i=9;i>0;i--){ig.game.spawnEntity(EntityParticle,x,y,{scale:{x:0.5,y:0.5},vel:{x:velocity*Math.cos(0.2*i),y:velocity*Math.sin(0.2*i)}})};velocity=25;for(var i=9;i>0;i--){ig.game.spawnEntity(EntityParticle,x,y,{scale:{x:0.8,y:0.8},vel:{x:velocity*Math.cos(-0.2*i),y:velocity*Math.sin(-0.2*i)}})};for(var i=9;i>0;i--){ig.game.spawnEntity(EntityParticle,x,y,{scale:{x:0.8,y:0.8},vel:{x:velocity*Math.cos(0.2*i),y:velocity*Math.sin(0.2*i)}})};velocity=15;for(var i=9;i>0;i--){ig.game.spawnEntity(EntityParticle,x,y,{vel:{x:velocity*Math.cos(-0.2*i),y:velocity*Math.sin(-0.2*i)}})};for(var i=9;i>0;i--){ig.game.spawnEntity(EntityParticle,x,y,{vel:{x:velocity*Math.cos(0.2*i),y:velocity*Math.sin(0.2*i)}})};ig.game.sortEntitiesDeferred();},createSmallParticleExplosion:function(x,y,xdir){var velocity=80;for(var i=5;i>0;i--){ig.game.spawnEntity(EntityParticle,x,y,{color:'white',vel:{x:xdir*velocity*Math.cos(-0.25*i),y:velocity*Math.sin(-0.2*i)}})};for(var i=5;i>0;i--){ig.game.spawnEntity(EntityParticle,x,y,{color:'white',vel:{x:xdir*velocity*Math.cos(0.25*i),y:velocity*Math.sin(0.2*i)}})};ig.game.sortEntitiesDeferred();},initKeys:function(){ig.input.bind(ig.KEY.M,'mute');ig.input.bind(ig.KEY.A,'left');ig.input.bind(ig.KEY.LEFT_ARROW,'left');ig.input.bind(ig.KEY.D,'right');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');ig.input.bind(ig.KEY.UP_ARROW,'up');ig.input.bind(ig.KEY.W,'up');ig.input.bind(ig.KEY.DOWN_ARROW,'down');ig.input.bind(ig.KEY.S,'down');ig.input.bind(ig.KEY.MOUSE1,'shoot');ig.input.bind(ig.KEY.SPACE,'shoot');ig.input.bind(ig.KEY.Z,'zip');ig.input.bind(ig.GAMEPAD1.PAD_LEFT,'left');ig.input.bind(ig.GAMEPAD1.PAD_RIGHT,'right');ig.input.bind(ig.GAMEPAD1.FACE_1,'zip');ig.input.bind(ig.GAMEPAD1.FACE_2,'zip');ig.input.bind(ig.GAMEPAD1.FACE_4,'zip');ig.input.bind(ig.GAMEPAD1.PAD_TOP,'up');ig.input.bind(ig.GAMEPAD1.PAD_BOTTOM,'down');},zoomTo:function(object,amount,speed,easing,onComplete){if(!speed){speed=2;}
if(!easing){easing=Power1.easeOut;}
TweenMax.killTweensOf(this);this.centerObject=object;if(onComplete){TweenMax.to(this,speed,{scaleFactor:amount,ease:easing,onComplete:onComplete});}else{TweenMax.to(this,speed,{scaleFactor:amount,ease:easing});}
this.zoomed=true;},setZoom:function(object,amount,zoomed){TweenMax.killTweensOf(this);ig.game.scaleFactor=amount;if(zoomed){ig.game.zoomed=true;}else{ig.game.zoomed=false;}},unZoom:function(){TweenMax.to(this,2,{scaleFactor:1});ig.game.zoomed=false;},getRandomY:function(y,range){var rand=Math.random();if(rand>0.5){y=y-Math.random()*range;}else{y=y+Math.random()*range;}
return y;},update:function(){gpads=navigator.getGamepads();if(gpads.length>0){this.gamepadSupport=true;}
ig.game.clearColor=ig.game.currentColor;this.parent();if(ig.game.backgroundSpawnTimer&&ig.game.backgroundSpawnTimer.delta()>0){this.spawnEntity(EntityNonPlayer,ig.game.player.pos.x-300,ig.game.getRandomY(ig.game.player.pos.y,150));this.spawnEntity(EntityDebris,ig.game.player.pos.x+500,ig.game.getRandomY(ig.game.player.pos.y,150));this.spawnEntity(EntityDebris,ig.game.player.pos.x+500,ig.game.getRandomY(ig.game.player.pos.y,150));this.spawnEntity(EntityDebris,ig.game.player.pos.x+500,ig.game.getRandomY(ig.game.player.pos.y,150));ig.game.backgroundSpawnTimer.reset();}
if(this.centerObject){TweenMax.to(this.screen,0.5,{x:this.centerObject.pos.x+this.centerObject.size.x/2-ig.system.width/2});TweenMax.to(this.screen,0.5,{y:this.centerObject.pos.y+this.centerObject.size.y/2-ig.system.height/2});}else if(ig.game.player){var offsetX=0;if(ig.game.player.vel.x>0){offsetX=150;}else{offsetX=-150;}
if(ig.game.zoomed){ig.game.unZoom();}
TweenMax.to(this.screen,0.5,{x:this.player.pos.x+offsetX-ig.system.width/2});TweenMax.to(this.screen,0.5,{y:this.player.pos.y-ig.system.height/2});}
if(ig.game.cameraShakeTimer&&ig.game.cameraShakeTimer.delta()<0){ig.game.cameraShake(10);}
if(ig.game.slowMotionTimer.delta()<0){ig.Timer.timeScale=0.33;this.setTimeScaleOnTweens(ig.Timer.timeScale);}else{if(ig.Timer.timeScale===0.33){this.setTimeScaleOnTweens(1);this.deactivateSlowMotion();}}
if(ig.game.dataPacketTimer&&ig.game.dataPacketTimer.delta()>0){ig.game.dataPacketTimer.reset();var sign=(Math.random()>0.5?1:-1);ig.game.spawnEntity(EntityDataPacket,ig.game.player.pos.x+600,ig.game.player.pos.y+Math.random()*300*sign)
ig.game.spawnEntity(EntityPlatform,ig.game.player.pos.x+600+Math.random()*150*sign,ig.game.player.pos.y+Math.random()*300*sign)}},activateSlowMotion:function(duration){ig.game.slowMotionTimer=new ig.Timer(duration);},deactivateSlowMotion:function(){ig.Timer.timeScale=1;},setTimeScaleOnTweens:function(timeScale){var tweens=TweenMax.getAllTweens();tweens.forEach(function(tween){tween._timeScale=timeScale;})},draw:function(){if(this.tweenColorsByTime){if(this.colorTweenTimer.delta()<0){ig.game.clearColor=this.tweenColorsTime(Math.abs(this.colorTweenTimer.delta()),this.tweenColorDuration,ig.game.currentColor,ig.game.newColor);}else{this.clearColor=ig.game.newColor;this.currentColor=ig.game.newColor;if(ig.game.colorTweenCallback){ig.game.colorTweenCallback();ig.game.colorTweenCallback=undefined;}}}
ig.system.context.save();ig.system.context.fillStyle=ig.game.clearColor;ig.system.context.fillRect(0,0,ig.system.width*ig.system.scale,ig.system.height*ig.system.scale);var scaleFactor=this.scaleFactor;ig.system.context.save();var newWidth=ig.system.width*scaleFactor;var newHeight=ig.system.height*scaleFactor;ig.system.context.translate(-(newWidth-ig.system.width)/2*ig.system.scale,-(newHeight-ig.system.height)/2*ig.system.scale);ig.system.context.scale(scaleFactor,scaleFactor);this.parent();ig.system.context.restore();ig.system.context.restore();},cameraShake:function(camShakePower){if(!camShakePower){this.camShakePower=5;}
var mod=1;if(Math.random()>0.5){mod=-1;}
ig.game.screen.x=ig.game.screen.x+Math.random()*3*mod;ig.game.screen.y=ig.game.screen.y+Math.random()*3*mod;},});ig.main('#canvas',MyGame,60,480,320,2);});