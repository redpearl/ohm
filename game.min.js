...
if(last===true){this.gainSuperPower();}},go:function(dir){this.direction=dir;},update:function(){if(!this.frozen){if(this.trailPointSpawnTimer.delta()>0){if(this.rippleTimer.delta()>0){var randDir=1;if(Math.random()<0.5){randDir=-1;}
this.lastPoint=ig.game.spawnEntity(EntityTrailPoint,this.pos.x+this.size.x/2,this.pos.y+this.size.y/2,{otherPoint:this.lastPoint,ripple:randDir*3*Math.random(),rippleDir:1});this.rippleTimer=new ig.Timer(Math.random()*0.5);}else{this.lastPoint=ig.game.spawnEntity(EntityTrailPoint,this.pos.x+this.size.x/2,this.pos.y+this.size.y/2,{otherPoint:this.lastPoint});}
...
this.accel.x=100;if(ig.input.state('down')&&!this.dead&&!this.winner){this.go('down');this.accel.y=500;}else if(ig.input.state('up')&&!this.dead&&!this.winner){this.go('up');this.accel.y=-500;}else{this.accel.y=0;}
if(ig.input.state('boost')){if(this.vel.x<0){this.accel.x=-600;}else{this.accel.x=600;}}
...
ig.baked=true;ig.module('game.entities.smallBlock').requires('impact.entity').defines(function(){EntitySmallBlock=ig.Entity.extend({size:{x:24,y:24},zIndex:251,gravityFactor:0,type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.A,collides:ig.Entity.COLLIDES.NEVER,animSheet:new ig.AnimationSheet('media/small_block.png',24,24),init:function(x,y,settings){this.parent(x,y,settings);var frame=Math.round(Math.random()*3);this.addAnim('idle',0.2,[frame]);var rotation=Math.random()/7;TweenMax.to(this.currentAnim,0.7,{angle:rotation,onComplete:this.rotateBack.bind(this)});TweenMax.to(this.pos,0.8,{x:this.pos.x+this.mod.x,y:this.pos.y+this.mod.y,onComplete:this.moveBack.bind(this)});},rotateBack:function(){TweenMax.to(this.currentAnim,0.25,{angle:0});},moveBack:function(){TweenMax.to(this.pos,0.25,{x:this.pos.x-this.mod.x,y:this.pos.y-this.mod.y});},update:function(){this.currentAnim.update();},draw:function(){this.parent();}});});

// lib/game/entities/block.js
ig.baked=true;ig.module('game.entities.block').requires('impact.entity','game.entities.smallBlock').defines(function(){EntityBlock=ig.Entity.extend({size:{x:48,y:48},zIndex:251,gravityFactor:0,zoomRadius:120,type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.A,collides:ig.Entity.COLLIDES.NEVER,animSheet:new ig.AnimationSheet('media/block.png',48,48),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.2,[0]);this.hitCooldown=new ig.Timer(0.1);},check:function(other){if(other===ig.game.player&&this.hitCooldown.delta()>0&&!ig.input.state('boost')&&!this.hasSplit){ig.game.player.vel.x=-ig.game.player.vel.x+-ig.game.player.vel.x/10;this.hitCooldown.reset();ig.game.player.hit('front');}else if(other===ig.game.player&&this.hitCooldown.delta()>0&&!this.hasSplit){this.hasSplit=true;ig.game.player.freeze();this.split();this.currentAnim.alpha=0;ig.game.sfxBoom.play();}},split:function(){ig.game.spawnEntity(EntitySmallBlock,this.pos.x,this.pos.y,{mod:{x:-2,y:-2}});ig.game.spawnEntity(EntitySmallBlock,this.pos.x+24,this.pos.y,{mod:{x:2,y:-2}});ig.game.spawnEntity(EntitySmallBlock,this.pos.x,this.pos.y+24,{mod:{x:-2,y:2}});ig.game.spawnEntity(EntitySmallBlock,this.pos.x+24,this.pos.y+24,{mod:{x:2,y:2}});},update:function(){if(this.distanceTo(ig.game.player)<this.zoomRadius||this===ig.game.player.closeObject){ig.game.player.closeObject=this;ig.game.player.zoomValue=this.zoomRadius/this.distanceTo(ig.game.player);}
this.currentAnim.update();},draw:function(){this.parent();}});});

// lib/game/entities/barrier.js
ig.baked=true;ig.module('game.entities.barrier').requires('impact.entity').defines(function(){EntityBarrier=ig.Entity.extend({size:{x:12,y:6000},zIndex:-10,gravityFactor:0,zoomRadius:60,type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.A,collides:ig.Entity.COLLIDES.NEVER,color:'#ddb7d5',init:function(x,y,settings){this.parent(x,y,settings);this.hitCooldown=new ig.Timer(0.1);},check:function(other){if(other===ig.game.player&&this.hitCooldown.delta()>0&&!ig.input.state('boost')&&!this.broken){ig.game.player.vel.x=-ig.game.player.vel.x+-ig.game.player.vel.x/10;this.hitCooldown.reset();ig.game.player.hit('front');}else if(!this.broken){this.broken=true;ig.game.sfxBoom.play();ig.game.activateSlowMotion(1);ig.game.createParticleExplosion(this.pos.x+2,other.pos.y+other.size.y/2);this.currentAnim.alpha=0.3;TweenMax.to(this,1,{alpha:1});}},isOnScreen:function(){if((this.pos.x>ig.game.screen.x&&this.pos.x<ig.game.screen.x+ig.system.width)||(this.pos.x+this.size.x>ig.game.screen.x&&this.pos.x+this.size.x>ig.game.screen.x+ig.system.width)){return true;}else if((this.pos.y>ig.game.screen.y&&this.pos.y<ig.game.screen.y+ig.system.height)||(this.pos.y+this.size.y>ig.game.screen.y&&this.pos.y+this.size.y>ig.game.screen.y+ig.system.height)){return true;}else{return false;}},update:function(){if(this.distanceTo(ig.game.player)<this.zoomRadius||this===ig.game.player.closeObject){ig.game.player.closeObject=this;ig.game.player.zoomValue=this.zoomRadius/this.distanceTo(ig.game.player);}},draw:function(){if(this.isOnScreen()){ig.system.context.globalAlpha=1;ig.system.context.fillStyle=ig.game.darkColor;ig.system.context.fillRect((this.pos.x-ig.game.screen.x)*ig.system.scale,(this.pos.y-ig.game.screen.y)*ig.system.scale,3000,6000);ig.system.context.fillStyle=ig.game.mediumDarkColor;ig.system.context.globalAlpha=0.8;ig.system.context.fillRect((this.pos.x-ig.game.screen.x)*ig.system.scale,(this.pos.y-ig.game.screen.y)*ig.system.scale,150-(this.pos.x-ig.game.player.pos.x)/4,6000);ig.system.context.globalAlpha=1;ig.system.context.fillStyle=this.color;ig.system.context.fillRect((this.pos.x-ig.game.screen.x)*ig.system.scale,(this.pos.y-ig.game.screen.y)*ig.system.scale,4,6000);}
this.parent();}});});

// lib/game/entities/storyText.js
ig.baked=true;ig.module('game.entities.storyText').requires('impact.entity').defines(function(){EntityStoryText=ig.Entity.extend({size:{x:480,y:320},zIndex:251,gravityFactor:0,zoomRadius:60,text:'Hello?',revealedText:'',cursor:'_',blinkTime:0.5,revealRatio:0.4,type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.A,collides:ig.Entity.COLLIDES.NEVER,init:function(x,y,settings){this.parent(x,y,settings);this.blinkTimer=new ig.Timer(this.blinkTime);this.textTimer=new ig.Timer(0);this.mainGameTimer=new ig.Timer(5);},update:function(){this.parent();this.calculateRevealedText();if(this.mainGameTimer.delta()>0&&!this.ended){var obj={pos:{x:35,y:10},size:{x:5,y:5}};ig.game.zoomTo(obj,0.1,7,Linear.noEase,function(){ig.game.loadGameMode('spark')});this.ended=true;}},draw:function(){if(this.blinkTimer.delta()>this.blinkTime){ig.game.font.draw(this.revealedText,this.pos.x-ig.game.screen.x+10,this.pos.y-ig.game.screen.y+10);this.blinkTimer.reset();}else if(this.blinkTimer.delta()>0){ig.game.font.draw(this.revealedText+this.cursor,this.pos.x-ig.game.screen.x+10,this.pos.y-ig.game.screen.y+10);}else{ig.game.font.draw(this.revealedText,this.pos.x-ig.game.screen.x+10,this.pos.y-ig.game.screen.y+10);}
this.parent();},calculateRevealedText:function(){var revealedChars=Math.floor(this.textTimer.delta()/this.revealRatio);if(revealedChars>=this.text.length){this.revealedText=this.text;this.revealed=true;}else{this.revealedText=this.text.substr(0,revealedChars);}}});});
...
// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.font','game.entities.player','game.entities.particle','game.entities.nonPlayer','game.entities.block','game.entities.barrier','game.entities.storyText','game.entities.debris','game.levels.test','game.levels.test','plugins.scalingPlugin').defines(function(){MyGame=ig.Game.extend({font:new ig.Font('media/courier16_white.png'),big:false,scaleFactor:1,clearColor:'#af0c27',darkColor:'#100710',mediumDarkColor:'#320221',init:function(){ig.game.initKeys();ig.game.loadMusicTracks();ig.game.loadSoundEffects();ig.game.slowMotionTimer=new ig.Timer(0);ig.game.loadGameMode('');},loadMusicTracks:function(){ig.music.add('media/music/theme.*','theme');ig.music.add('media/music/atmospheric.*','atmospheric');ig.music.add('media/music/atmospheric2.*','atmospheric2');ig.music.add('media/music/interference.*','interference');ig.music.add('media/music/interference2.*','interference2');ig.music.add('media/music/transmission.*','transmission');ig.music.add('media/music/transmission2.*','transmission2');ig.music.loop=true;ig.music.volume=0.5;},loadSoundEffects:function(){ig.game.sfxBoom=new ig.Sound('media/sound/boom.*');},loadGameMode:function(mode){if(mode==='spark'){ig.music.play('atmospheric2');ig.game.mode=mode;ig.game.scaleFactor=1;ig.game.zoomed=false;this.clearColor='#af0c27';ig.game.loadLevel(LevelTest);ig.game.player=ig.game.spawnEntity(EntityPlayer,100,100);ig.game.spawnEntity(EntityBlock,250,100);ig.game.spawnEntity(EntityBarrier,550,-1500);ig.game.backgroundSpawnTimer=new ig.Timer(1.5);}else{ig.music.play('transmission2');ig.game.mode=mode;this.clearColor='#000000';ig.game.loadLevel(LevelTest);ig.game.spawnEntity(EntityStoryText,0,0);}},createParticleExplosion:function(x,y){var velocity=35;for(var i=9;i>0;i--){ig.game.spawnEntity(EntityParticle,x,y,{scale:{x:0.5,y:0.5},vel:{x:velocity*Math.cos(-0.2*i),y:velocity*Math.sin(-0.2*i)}})};for(var i=9;i>0;i--){ig.game.spawnEntity(EntityParticle,x,y,{scale:{x:0.5,y:0.5},vel:{x:velocity*Math.cos(0.2*i),y:velocity*Math.sin(0.2*i)}})};var velocity=25;for(var i=9;i>0;i--){ig.game.spawnEntity(EntityParticle,x,y,{scale:{x:0.8,y:0.8},vel:{x:velocity*Math.cos(-0.2*i),y:velocity*Math.sin(-0.2*i)}})};for(var i=9;i>0;i--){ig.game.spawnEntity(EntityParticle,x,y,{scale:{x:0.8,y:0.8},vel:{x:velocity*Math.cos(0.2*i),y:velocity*Math.sin(0.2*i)}})};var velocity=15;for(var i=9;i>0;i--){ig.game.spawnEntity(EntityParticle,x,y,{vel:{x:velocity*Math.cos(-0.2*i),y:velocity*Math.sin(-0.2*i)}})};for(var i=9;i>0;i--){ig.game.spawnEntity(EntityParticle,x,y,{vel:{x:velocity*Math.cos(0.2*i),y:velocity*Math.sin(0.2*i)}})};ig.game.sortEntitiesDeferred();},initKeys:function(){ig.input.bind(ig.KEY.M,'mute');ig.input.bind(ig.KEY.A,'left');ig.input.bind(ig.KEY.LEFT_ARROW,'left');ig.input.bind(ig.KEY.D,'right');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');ig.input.bind(ig.KEY.UP_ARROW,'up');ig.input.bind(ig.KEY.W,'up');ig.input.bind(ig.KEY.DOWN_ARROW,'down');ig.input.bind(ig.KEY.S,'down');ig.input.bind(ig.KEY.SPACE,'boost');},zoomTo:function(object,amount,speed,easing,onComplete){if(!speed){speed=2;}
