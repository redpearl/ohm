...
if(last===true){this.gainSuperPower();}},go:function(dir){this.direction=dir;},update:function(){if(this.trailPointSpawnTimer.delta()>0){if(this.rippleTimer.delta()>0){var randDir=1;if(Math.random()<0.5){randDir=-1;}
...
ig.baked=true;ig.module('game.entities.block').requires('impact.entity').defines(function(){EntityBlock=ig.Entity.extend({size:{x:24,y:24},zIndex:251,gravityFactor:0,zoomRadius:60,type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.A,collides:ig.Entity.COLLIDES.NEVER,animSheet:new ig.AnimationSheet('media/block.png',24,24),init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.2,[0]);this.hitCooldown=new ig.Timer(0.1);},check:function(other){if(other===ig.game.player&&this.hitCooldown.delta()>0){ig.game.player.vel.x=-ig.game.player.vel.x+-ig.game.player.vel.x/10;this.hitCooldown.reset();ig.game.player.hit('front');}},update:function(){if(this.distanceTo(ig.game.player)<this.zoomRadius||this===ig.game.player.closeObject){ig.game.player.closeObject=this;ig.game.player.zoomValue=this.zoomRadius/this.distanceTo(ig.game.player);}
this.currentAnim.update();},draw:function(){this.parent();}});});

// lib/game/entities/barrier.js
ig.baked=true;ig.module('game.entities.barrier').requires('impact.entity').defines(function(){EntityBarrier=ig.Entity.extend({size:{x:12,y:6000},zIndex:-10,gravityFactor:0,zoomRadius:60,type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.A,collides:ig.Entity.COLLIDES.NEVER,color:'#ddb7d5',init:function(x,y,settings){this.parent(x,y,settings);this.hitCooldown=new ig.Timer(0.1);},check:function(other){if(other===ig.game.player&&this.hitCooldown.delta()>0&&!ig.input.state('boost')){ig.game.player.vel.x=-ig.game.player.vel.x+-ig.game.player.vel.x/10;this.hitCooldown.reset();ig.game.player.hit('front');}else{}},update:function(){if(this.distanceTo(ig.game.player)<this.zoomRadius||this===ig.game.player.closeObject){ig.game.player.closeObject=this;ig.game.player.zoomValue=this.zoomRadius/this.distanceTo(ig.game.player);}},draw:function(){ig.system.context.globalAlpha=1;ig.system.context.fillStyle=ig.game.darkColor;ig.system.context.fillRect((this.pos.x-ig.game.screen.x)*ig.system.scale,(this.pos.y-ig.game.screen.y)*ig.system.scale,3000,6000);ig.system.context.fillStyle=ig.game.mediumDarkColor;ig.system.context.globalAlpha=0.8;ig.system.context.fillRect((this.pos.x-ig.game.screen.x)*ig.system.scale,(this.pos.y-ig.game.screen.y)*ig.system.scale,150-(this.pos.x-ig.game.player.pos.x)/4,6000);ig.system.context.globalAlpha=1;ig.system.context.fillStyle=this.color;ig.system.context.fillRect((this.pos.x-ig.game.screen.x)*ig.system.scale,(this.pos.y-ig.game.screen.y)*ig.system.scale,4,6000);this.parent();}});});

// lib/game/entities/storyText.js
ig.baked=true;ig.module('game.entities.storyText').requires('impact.entity').defines(function(){EntityStoryText=ig.Entity.extend({size:{x:480,y:320},zIndex:251,gravityFactor:0,zoomRadius:60,text:'Hello?',revealedText:'',cursor:'_',blinkTime:0.5,revealRatio:0.4,type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.A,collides:ig.Entity.COLLIDES.NEVER,init:function(x,y,settings){this.parent(x,y,settings);this.blinkTimer=new ig.Timer(this.blinkTime);this.textTimer=new ig.Timer(0);this.mainGameTimer=new ig.Timer(5);},update:function(){this.parent();this.calculateRevealedText();if(this.mainGameTimer.delta()>0&&!this.ended){this.ended=true;ig.game.loadGameMode('spark');}},draw:function(){this.parent();if(this.blinkTimer.delta()>this.blinkTime){ig.game.font.draw(this.revealedText,this.pos.x+10,this.pos.y+10);this.blinkTimer.reset();}else if(this.blinkTimer.delta()>0){ig.game.font.draw(this.revealedText+this.cursor,this.pos.x+10,this.pos.y+10);}else{ig.game.font.draw(this.revealedText,this.pos.x+10,this.pos.y+10);}},calculateRevealedText:function(){var revealedChars=Math.floor(this.textTimer.delta()/this.revealRatio);if(revealedChars>=this.text.length){this.revealedText=this.text;this.revealed=true;}else{this.revealedText=this.text.substr(0,revealedChars);}}});});
...
// lib/game/main.js
ig.baked=true;ig.module('game.main').requires('impact.game','impact.font','game.entities.player','game.entities.nonPlayer','game.entities.block','game.entities.barrier','game.entities.storyText','game.entities.debris','game.levels.test','plugins.scalingPlugin').defines(function(){MyGame=ig.Game.extend({font:new ig.Font('media/courier16_green.png'),big:false,scaleFactor:1,clearColor:'#af0c27',darkColor:'#100710',mediumDarkColor:'#320221',init:function(){ig.game.initKeys();ig.game.loadGameMode('sark');},loadGameMode:function(mode){if(mode==='spark'){ig.game.mode=mode;this.clearColor='#af0c27';ig.game.loadLevel(LevelTest);ig.game.player=ig.game.spawnEntity(EntityPlayer,100,100);ig.game.spawnEntity(EntityBlock,550,100);ig.game.spawnEntity(EntityBarrier,1550,-1500);ig.game.backgroundSpawnTimer=new ig.Timer(1.5);}else{ig.game.mode=mode;this.clearColor='#000000';ig.game.loadLevel(LevelTest);ig.game.spawnEntity(EntityStoryText,0,0);}},initKeys:function(){ig.input.bind(ig.KEY.M,'mute');ig.input.bind(ig.KEY.A,'left');ig.input.bind(ig.KEY.LEFT_ARROW,'left');ig.input.bind(ig.KEY.D,'right');ig.input.bind(ig.KEY.RIGHT_ARROW,'right');ig.input.bind(ig.KEY.UP_ARROW,'up');ig.input.bind(ig.KEY.W,'up');ig.input.bind(ig.KEY.DOWN_ARROW,'down');ig.input.bind(ig.KEY.S,'down');ig.input.bind(ig.KEY.SPACE,'boost');},zoomTo:function(object,amount){TweenMax.killTweensOf(this);this.centerObject=object;TweenMax.to(this,2,{scaleFactor:amount});this.zoomed=true;},unZoom:function(){TweenMax.to(this,2,{scaleFactor:1});ig.game.zoomed=false;},getRandomY:function(y,range){var rand=Math.random();if(rand>0.5){y=y-Math.random()*range;}else{y=y+Math.random()*range;}
...
if(ig.game.cameraShakeTimer&&ig.game.cameraShakeTimer.delta()<0){ig.game.cameraShake(10);}},draw:function(){var scaleFactor=this.scaleFactor;ig.system.context.save();var newWidth=ig.system.width*scaleFactor;var newHeight=ig.system.height*scaleFactor;ig.system.context.translate(-(newWidth-ig.system.width)/2*ig.system.scale,-(newHeight-ig.system.height)/2*ig.system.scale);ig.system.context.scale(scaleFactor,scaleFactor);this.parent();ig.system.context.restore();},cameraShake:function(camShakePower){if(!camShakePower){this.camShakePower=5;}
var mod=1;if(Math.random()>0.5){mod=-1;}
ig.game.screen.x=Math.random()*3*mod;ig.game.screen.y=Math.random()*3*mod;},});ig.main('#canvas',MyGame,60,480,320,2);});