...
ig.system.context.strokeStyle='#f8f4f8';ig.system.context.moveTo((this.pos.x-ig.game.screen.x)*ig.system.scale,(this.pos.y+this.ripple-ig.game.screen.y)*ig.system.scale);ig.system.context.lineTo((this.pos.x-ig.game.screen.x)*ig.system.scale,(this.otherPoint.pos.y+this.ripple-ig.game.screen.y)*ig.system.scale);ig.system.context.stroke();if(!this.small){if(this.lifeTimer.delta()>-this.deathTime/2){ig.system.context.lineWidth=2;}else{ig.system.context.lineWidth=4;}}
...
ig.baked=true;ig.module('game.entities.player').requires('impact.entity','game.entities.bounceable','game.entities.trailPoint').defines(function(){EntityPlayer=EntityBounceable.extend({size:{x:16,y:4},maxVel:{x:150,y:150},maximumVelocity:150,offset:{x:0,y:0},friction:{x:0,y:350},zIndex:100,gravityFactor:0,active:true,superPowerDuration:22,streak:0,score:0,type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.NONE,collides:ig.Entity.COLLIDES.PASSIVE,animSheet:new ig.AnimationSheet('media/player_sprite.png',16,4),glowImage:new ig.Image('media/player_glow.png',16,16),flip:false,accelGround:500,accelAir:500,jump:130,init:function(x,y,settings){this.parent(x,y,settings);this.addAnim('idle',0.2,[0]);this.trailPointSpawnTimer=new ig.Timer(0.1);this.rippleTimer=new ig.Timer(Math.random());this.forceBoostTimer=new ig.Timer(0);ig.game.screen.x=this.pos.x-ig.system.width/2;ig.game.screen.y=this.pos.y-ig.system.height/2;},freeze:function(){this.frozen=true;this.currentAnim.alpha=0;},activate:function(){this.frozen=false;this.currentAnim.alpha=1;},draw:function(){if(this.lastPoint&&!this.lastPoint._killed&&!this.frozen){ig.system.context.globalAlpha=1;ig.system.context.beginPath();ig.system.context.lineWidth=4;ig.system.context.strokeStyle='#f8f4f8';ig.system.context.moveTo((this.pos.x-ig.game.screen.x)*ig.system.scale,(this.pos.y-ig.game.screen.y)*ig.system.scale);ig.system.context.moveTo((this.pos.x-ig.game.screen.x)*ig.system.scale,(this.lastPoint.pos.y-ig.game.screen.y)*ig.system.scale);ig.system.context.lineTo((this.lastPoint.pos.x-ig.game.screen.x)*ig.system.scale,(this.lastPoint.pos.y-ig.game.screen.y)*ig.system.scale);ig.system.context.stroke();ig.system.context.globalAlpha=1;}
...
this.accel.x=100;if(ig.input.state('down')&&!this.dead&&!this.winner&&this.forceBoostTimer.delta()>0){this.go('down');this.accel.y=500;}else if(ig.input.state('up')&&!this.dead&&!this.winner&&this.forceBoostTimer.delta()>0){this.go('up');this.accel.y=-500;}else{this.accel.y=0;}
if(ig.input.state('boost')&&this.vel.x>0){this.boosting=true;this.scale.y=1.3;this.scale.x=0.6;this.accel.x=600;this.maxVel.x=this.maximumVelocity+this.maximumVelocity/10;}else if(this.forceBoostTimer.delta()<0){this.superBoosting=true;this.scale.y=1.6;this.scale.x=0.3;this.accel.x=600;this.maxVel.x=this.maximumVelocity*2;}else{this.boosting=false;this.superBoosting=false;this.scale.y=1;this.scale.x=1;this.maxVel.x=this.maximumVelocity;}
this.currentAnim.flip.x=this.flip;this.parent();}},hit:function(direction){if(direction==='front'){this.stretch(0.3,0.3);}}});});
...
ig.baked=true;ig.module('game.entities.storyText').requires('impact.entity').defines(function(){EntityStoryText=ig.Entity.extend({size:{x:480,y:320},zIndex:251,gravityFactor:0,zoomRadius:60,text:'Hello?',revealedText:'',cursor:'_',blinkTime:0.5,revealRatio:0.4,charactersWritten:0,type:ig.Entity.TYPE.A,checkAgainst:ig.Entity.TYPE.A,collides:ig.Entity.COLLIDES.NEVER,init:function(x,y,settings){this.parent(x,y,settings);this.blinkTimer=new ig.Timer(this.blinkTime);this.textTimer=new ig.Timer(0);this.mainGameTimer=new ig.Timer(5);},update:function(){this.parent();this.calculateRevealedText();if(this.mainGameTimer.delta()>0&&!this.ended){var obj={pos:{x:35,y:10},size:{x:5,y:5}};ig.game.zoomTo(obj,0.1,7,Linear.noEase,function(){ig.game.loadGameMode('spark')});ig.game.tweenColorsByTime=true;ig.game.colorTweenTimer=new ig.Timer(6);this.ended=true;}},draw:function(){if(this.blinkTimer.delta()>this.blinkTime){ig.game.font.draw(this.revealedText,this.pos.x-ig.game.screen.x+10,this.pos.y-ig.game.screen.y+10);this.blinkTimer.reset();}else if(this.blinkTimer.delta()>0){ig.game.font.draw(this.revealedText+this.cursor,this.pos.x-ig.game.screen.x+10,this.pos.y-ig.game.screen.y+10);}else{ig.game.font.draw(this.revealedText,this.pos.x-ig.game.screen.x+10,this.pos.y-ig.game.screen.y+10);}
